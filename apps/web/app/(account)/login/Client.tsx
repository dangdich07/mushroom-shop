      ...
      <main className="w-full max-w-md">
        <div className="mb-6 text-center space-y-1.5">
          <div className="inline-flex items-center justify-center gap-2">
            ...
            <span className="text-xl font-semibold tracking-tight">Mushroom Shop</span>
          </div>
          <h1 className="text-2xl font-bold mt-2">Đăng nhập tài khoản</h1>
          <p className="text-sm text-gray-600">Chào mừng bạn quay lại. Đăng nhập để tiếp tục.</p>
        </div>
        ...
        <div className="rounded-2xl bg-white/90 border border-gray-100 shadow-lg shadow-stone-200 px-6 py-6 space-y-4 backdrop-blur-sm">
          <form onSubmit={onSubmit} className="space-y-4">
            ...
            <div className="space-y-1.5">
              <label htmlFor="password" className="block text-xs font-medium text-gray-700 uppercase tracking-wide">Mật khẩu</label>
            ...
            <button disabled={loading} className="mt-3 w-full rounded-full bg-stone-900 py-2.5 text-sm font-semibold text-white shadow-sm transition hover:bg-black disabled:opacity-60">
          ...
          <div className="flex items-center justify-between text-xs text-stone-500">
            <span />
            <Link href="/forgot-password" className="hover:text-stone-800 hover:underline">Quên mật khẩu?</Link>
          </div>
        </div>

        <p className="mt-4 text-center text-sm text-gray-600">
          Chưa có tài khoản? <Link href="/register" className="font-semibold text-gray-900 hover:underline">Đăng ký ngay</Link>
        </p>
      </main>
    </div>
  );
